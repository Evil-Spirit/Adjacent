...
ExpVector::ExpVector(const std::shared_ptr<Expr>& x,
...
{
	return std::abs(x->eval() - o.x->eval()) < eps &&
		   std::abs(y->eval() - o.y->eval()) < eps &&
...
std::shared_ptr<Expr> PointLineDistance(const ExpVector& point, const ExpVector& l0, const ExpVector& l1) {
	auto d = l0 - l1;
...
ExpVector rotate_around(const ExpVector& point, const ExpVector& axis, const ExpVector& origin, const std::shared_ptr<Expr>& angle) {
	auto a = axis.normalized();
	auto c = cos(angle);
	auto s = sin(angle);
	auto u = ExpVector(c + (one - c) * a.x * a.x, (one - c) * a.y * a.x + s * a.z, (one - c) * a.z * a.x - s * a.y);
	auto v = ExpVector((one - c) * a.x * a.y - s * a.z, c + (one - c) * a.y * a.y, (one - c) * a.z * a.y + s * a.x);
	auto n = ExpVector((one - c) * a.x * a.z + s * a.y, (one - c) * a.y * a.z - s * a.x,	c + (one - c) * a.z * a.z);
	auto p = point - origin;
...
// 	auto u = Vector3(c + (1 - c) * a.x * a.x, (1 - c) * a.y * a.x + s * a.z, (1 - c) * a.z * a.x - s * a.y);
// 	auto v = Vector3((1 - c) * a.x * a.y - s * a.z, c + (1 - c) * a.y * a.y, (1 - c) * a.z * a.y + s * a.x);
// 	auto n = Vector3((1 - c) * a.x * a.z + s * a.y, (1 - c) * a.y * a.z - s * a.x, c + (1 - c) * a.z * a.z);
// 	auto p = point - origin;
// 	return p.x * u + p.y * v + p.z * n + origin;
// }
