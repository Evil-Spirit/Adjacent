...
std::shared_ptr<Expr> operator-(const std::shared_ptr<Expr>& a) {
	if(a->is_zero_const()) return a;
	if(a->is_const()) return std::make_shared<Expr>(-a->value);
...
std::shared_ptr<Expr> operator-(const std::shared_ptr<Expr>& a, const std::shared_ptr<Expr>& b) {
	if(a->is_zero_const()) return -b;
	if(b->is_zero_const()) return a;
...
std::shared_ptr<Expr> operator*(const std::shared_ptr<Expr>& a, const std::shared_ptr<Expr>& b) {
	if(a->is_zero_const()) return zero;
	if(b->is_zero_const()) return zero;
	if(a->is_one_const()) return b;
	if(b->is_one_const()) return a;
	if(a->is_minus_one_const()) return -b;
	if(b->is_minus_one_const()) return -a;
	if(a->is_const() && b->is_const()) return std::make_shared<Expr>(a->value * b->value);
...
std::shared_ptr<Expr> sin (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Sin, x, nullptr); }
std::shared_ptr<Expr> cos (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Cos, x, nullptr); }
std::shared_ptr<Expr> acos (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::ACos, x, nullptr); }
std::shared_ptr<Expr> asin (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::ASin, x, nullptr); }
std::shared_ptr<Expr> sqrt (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Sqrt, x, nullptr); }
std::shared_ptr<Expr> sqr (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Sqr, x, nullptr); }
std::shared_ptr<Expr> abs (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Abs, x, nullptr); }
std::shared_ptr<Expr> sign (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Sign, x, nullptr); }
std::shared_ptr<Expr> atan2 (const std::shared_ptr<Expr>& x, const std::shared_ptr<Expr>& y) { return std::make_shared<Expr>(Op::Atan2, x, y); }
std::shared_ptr<Expr> expo (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Exp, x, nullptr); }
std::shared_ptr<Expr> sinh (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Sinh, x, nullptr); }
std::shared_ptr<Expr> cosh (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::Cosh, x, nullptr); }
std::shared_ptr<Expr> sfres (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::SFres,	x, nullptr); }
std::shared_ptr<Expr> cfres (const std::shared_ptr<Expr>& x) { return std::make_shared<Expr>(Op::CFres,	x, nullptr); }


// Todo automatic conversion from double?!
Expr::Expr(double value) :
	value(value), op(Op::Const)
{
}
...
double Expr::eval()
{
	switch(op) {
		...
		case Op::ParamOp:	return param->value();
		...
		case Op::Sin:	return std::sin(a->eval());
		case Op::Cos:	return std::cos(a->eval());
		case Op::ACos:	return std::acos(a->eval());
		case Op::ASin:	return std::asin(a->eval());
		case Op::Sqrt:	return std::sqrt(a->eval());
		case Op::Sqr:	{  double av = a->eval(); return av * av; }
		case Op::Atan2:	return std::atan2(a->eval(), b->eval());
		case Op::Abs:	return std::abs(a->eval());
		// TODO figure out sign
		// case Op::Sign:	return std::sign(a->eval());
		case Op::Neg:	return -a->eval();
		case Op::Pos:	return a->eval();
		case Op::Exp:	return std::exp(a->eval());
		case Op::Sinh:	return std::sinh(a->eval());
		case Op::Cosh:	return std::cosh(a->eval());
...
std::string Expr::quoted() {
	if(is_unary()) return to_string();
	return "(" + to_string() + ")";
}

std::string Expr::quoted_add() {
	if(!is_additive()) return to_string();
	return "(" + to_string() + ")";
}

std::string Expr::to_string() {
	switch(op) {
		case Op::Const: return std::to_string(value);
		case Op::ParamOp: return param->m_name;
		case Op::Add: return a->to_string() + " + " + b->to_string();
		case Op::Sub: return a->to_string() + " - " + b->quoted_add();
		case Op::Mul: return a->quoted_add() + " * " + b->quoted_add();
		case Op::Div: return a->quoted_add() + " / " + b->quoted();
		case Op::Sin: return "sin(" + a->to_string() + ")";
		case Op::Cos: return "cos(" + a->to_string() + ")";
		case Op::ASin: return "asin(" + a->to_string() + ")";
		case Op::ACos: return "acos(" + a->to_string() + ")";
		case Op::Sqrt: return "sqrt(" + a->to_string() + ")";
		case Op::Sqr: return a->quoted() + " ^ 2";
		case Op::Abs: return "abs(" + a->to_string() + ")";
		case Op::Sign: return "sign(" + a->to_string() + ")";
		case Op::Atan2: return "atan2(" + a->to_string() + ", " + b->to_string() + ")";
		case Op::Neg: return "-" + a->quoted();
		case Op::Pos: return "+" + a->quoted();
		case Op::Drag:   return a->to_string() + " ≈ " + b->quoted_add();
		case Op::Exp: return "exp(" + a->to_string() + ")";
		case Op::Sinh: return "sinh(" + a->to_string() + ")";
		case Op::Cosh: return "cosh(" + a->to_string() + ")";
		case Op::SFres: return "sfres(" + a->to_string() + ")";
		case Op::CFres: return "cfres(" + a->to_string() + ")";
...
std::shared_ptr<Expr> Expr::d(const std::shared_ptr<Param<double>>& p) {
	switch(op) {
		...
		case Op::SFres: return a->d(p) * sin(expr(M_PI) * sqr(a) / two);
		case Op::CFres: return a->d(p) * cos(expr(M_PI) * sqr(a) / two);
	}
	return zero;
}
